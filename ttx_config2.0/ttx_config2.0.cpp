	...
	case  WM_INITDIALOG:
		{
			...
			HWND hFullscreen = GetDlgItem(hwnd, IDC_CHECK_FULLSCREEN);

			HWND hWindowedCheck = GetDlgItem(hwnd, IDC_CHECK_FULLSCREEN);
		...
		case IDC_CHECK_FULLSCREEN:
			{
				...
				{
				case BN_CLICKED:
					if (Button_GetCheck(GetDlgItem(hwnd, IDC_CHECK_FULLSCREEN)) == BST_CHECKED) {
						configMgr.SetConfig(TTX_CONFIG_FULLSCREEN, 1);
					} else {
						configMgr.SetConfig(TTX_CONFIG_FULLSCREEN, 0);
					}
					break;
